...
<c:if test="${not empty resultsMap}">
                            ...
                            <tr>
                                <th colspan="2" class="table-category-header">
                                    ...
                                    <c:choose>
                                        ...
                                        <c:when test="${category == 'sociodemograficas'}">Características Sociodemográficas</c:when>
    ...
    <c:choose>
        ...
        <%-- Si el ámbito es "Rural", mostrar "Rural" y omitir las otras filas --%>
        <c:otherwise>
            <tr>
                <th>Ámbito</th>
                <td>Rural</td>
    ...
    <c:if test="${categoryData['Nomenclatura Cartográfica Rural'] != null}">
        <tr>
            ...
            <td><c:out value="${categoryData['Nomenclatura Cartográfica Rural'][0]}" /></td>
                                ...
                                <c:when test="${category == 'sociodemograficas'}">
                                    <%-- Lógica para 'sociodemograficas' --%>
                                    <c:set var="orderList" value="${['Población total',
                                                                    'Población femenina',
                                                                    'Población masculina',
                                ...
                                <c:when test="${category == 'cuerposAgua'}">
                                    <%--<tr>
                                        <th colspan="2">Cuerpos de Agua Cercanos</th>
                                    </tr>
                                    --%>
                                    ...
                                    <c:forEach var="subEntry" items="${categoryData}">
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <!-- Otros subcategorías como 'Número de fallas cercanas' -->
    ...
    <!-- ============ 1. RESTRICCIONES ============ -->
    <c:if test="${categoryData['Áreas Protegidas'] != null
                 || categoryData['Frontera']        != null}">
        <tr>
        ...
        <!-- Frontera -->
        <c:if test="${categoryData['Frontera'] != null}">
...
<!-- ============== 2. DERECHOS =============== -->
<c:if test="${categoryData['Derechos_CURT']        != null
             || categoryData['CURT_Parcela']       != null
             || categoryData['Derechos_Titularidad'] != null
             || categoryData['Tierras de uso común'] != null
             || (not empty sessionScope.banobras_sedesol_label and not empty sessionScope.catastroCurtResult)
}">
    <tr>
        <th colspan="2" class="table-subheader">Derechos</th>
    </tr>

    <!-- SOLO mostrar uno u otro -->
    <c:choose>
        
        <c:when test="${categoryData['Derechos_CURT'] != null}">
            <tr>
                <th>CURT del predio</th>
                <td><c:out value="${categoryData['Derechos_CURT'][0]}"/></td>
            </tr>
        </c:when>
    ...
    <!-- CURT de la parcela -->
    <c:if test="${categoryData['CURT_Parcela'] != null}">
        <tr>
            <th>CURT de la parcela</th>
            <td><c:out value="${categoryData['CURT_Parcela'][0]}"/></td>
        </tr>
    </c:if>

    <!-- Titularidades -->
    <c:if test="${categoryData['Derechos_Titularidad'] != null}">
        <c:forEach var="titularidad" items="${categoryData['Derechos_Titularidad']}">
            <tr>
                <th>Titularidad</th>
    ...
    <!-- Tierras de uso común -->
    <c:if test="${categoryData['Tierras de uso común'] != null}">
        <c:forEach var="item" items="${categoryData['Tierras de uso común']}" varStatus="st">
            <tr>
                <th>
                    <c:choose>
                ...
                <td><c:out value="${item}" /></td>
                                ...
                                <c:when test="${category == 'geodesia'}">
                                    ...
                                    <!-- Bloque para Estación Gravimétrica más cercana -->
<tr>
    <th colspan="2" class="table-subheader">
...
<!-- Verificar si hay datos de la estación gravimétrica más cercana -->
        ...
        <c:if test="${status.index < 2}">
            <tr>
                ...
                <td><c:out value="${item}" escapeXml="false" /></td>
                                    ...
                                    <!-- Bloque para Vértice de posicionamiento más cercano -->
<tr>
    <th colspan="2" class="table-subheader">
...
<!-- Verificar si hay datos del vértice de posicionamiento más cercano -->
...
<script>
    ...
    // ***** PASO 1: DEFINIR LA PROYECCIÓN A REPROYECTAR *****
    // Le "enseñamos" a OpenLayers qué es EPSG:6365 para que pueda hacer los cálculos.
    ...
    // Definimos el área de cobertura (extent) de esta proyección para que OpenLayers sepa cómo alinearla.
    const projection6365 = ol.proj.get('EPSG:6365');
    projection6365.setExtent([1486369, 437635, 3513630, 1562364]);
    ...
    // Definimos una variable con la configuración estándar de la grilla 900913/3857
    // para no repetir código.
    const standardWebMercatorGrid = new ol.tilegrid.WMTS({
        origin: [-20037508.3428, 20037508.3428], // Origen estándar para esta proyección
        resolutions: [
            156543.03392804097, 78271.51696402048, 39135.75848201024,
            19567.87924100512, 9783.93962050256, 4891.96981025128,
            2445.98490512564, 1222.99245256282, 611.49622628141,
        ...
        // Usamos el nombre que tus capas de INEGI usan
    ...
    const gaiaWMSResult = new ol.layer.Tile({
        ...
        source: new ol.source.WMTS({
            ...
            tileGrid: standardWebMercatorGrid, // Usamos la cuadrícula estándar
            wrapX: true
        })
    });
    ...
    const mapaBaseHipsografico = new ol.layer.Tile({
        ...
        source: new ol.source.WMTS({
            ...
            matrixSet: 'EPSG:900913',
            format: 'image/jpeg',
            tileGrid: standardWebMercatorGrid // Reutilizamos la grilla estándar
        }),
...
const sidaisarTileGrid6365 = new ol.tilegrid.WMTS({
        // El origen debe ser la esquina superior izquierda del extent de la proyección
        origin: ol.extent.getTopLeft(projection6365.getExtent()),
        // Las resoluciones se extraen de los ScaleDenominator del XML
        resolutions: [
            16832.02881785637, 8416.014408928185, 4208.0072044640925, 
            2104.0036022320463, 1052.0018011160231, 526.0009005580116, 
            263.0004502790058, 131.5002251395029, 65.75011256975145, 
            32.875056284875725, 16.437528142437863, 8.218764071218931, 
            4.109382035609466, 2.054691017804733, 1.0273455089023665, 
            0.5136727544511832, 0.2568363772255916
        ],
        matrixIds: Array.from({length: 17}, (v, k) => 'EPSG:6365:' + k),
        tileSize: 512 // ***** El detalle crucial del XML *****
    });
    ...
    const sidaisarWmtsLayer = new ol.layer.Tile({
        ...
        type: 'base',
        source: new ol.source.WMTS({
            url: 'http://10.153.3.20:85/geowebcache/service/wmts',
            layer: 'SIDAISAR',
            matrixSet: 'EPSG:6365', // Pedimos la única que el servidor nos da bien
            format: 'image/png',
            projection: 'EPSG:6365', // Indicamos la proyección de la capa
            tileGrid: sidaisarTileGrid6365, // Usamos la cuadrícula 100% correcta
            style: ''
        })
    });
...
// ¿Debo mostrar processedParcel o banobras?
...
// Lógica exclusiva: solo muestra uno
if (showProcessedParcel) {
%>
    // Procesar solo processedParcelGeometryJson
...
%>
    // Procesar solo catastroGeomResult
    ...
    if (banobrasGeom) {
        ...
        const banobrasLayer = new ol.layer.Vector({
            ...
            style : new ol.style.Style({
                stroke: new ol.style.Stroke({
                    color: 'purple',        // ← Cambia el color que prefieras
                    width: 3
                }),
                fill  : new ol.style.Fill({
                    color: 'rgba(128,0,128,0.18)'   // ← Relleno traslúcido, opcional
                })
            })
        });
