        ...
        <div id="nav">
          <ul class="text-left">
           ...
           <ul class="text-right">
              ...
              </li><li class="cerrar_ses"> <a href="logout_curt.jsp"><span class="icon-exit"></span>   Cerrar Sesión</a></li>
           </ul>
        </div>
   </header>
                ...
                <a class="boton_delete" href="javascript:postwith('elimina_registro.jsp',{ue:'<%=id_ue%>', tab:'capacitacion'})"> 
                   ...
                   <div id="cont_contacto"> 
                     <%
                         ...
                         try{            
                    PreparedStatement pst28 =(PreparedStatement) conexion.conn.prepareStatement( "SELECT id_solic,  TO_CHAR(fecha_solicitud, 'dd/MM/yyyy') as fecha_solicitud, id_ue, nombre_solicitante, correo, "
                            + "tipo_solicitud, estatus, mensaje "
                            + "FROM \"seguim_CURT\".solicitud_soporte WHERE id_ue = '"+id_ue+"' and estatus = 'PENDIENTE'" ,ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                        ...
                        if(conexion.rs.next()){//SI encontró algo imprimimos lso encabezados...
                           ...
                           while(conexion.rs.next()) {
                               ...
                               estatus=conexion.rs.getString("estatus"); 
                            
                     %>
                            
                             Se envió una solicitud para desbloquear este módulo.<br>
                             Fecha solicitud: <%=fecha_solicitud%><br>
                             Estatus: <%=estatus%><br><br>
                       ...
                       <%}//while
                       %>
                            <a href="contacto_admin.jsp?ue=<%=id_ue%>&tipo_solicitud=libera_capacitacion">Volver a enviar</a>
                       <% }//if
                        else{ %>
                          La fecha límite para completar esta parte del formulario ha caducado contacta con el <a href="contacto_admin.jsp?ue=<%=id_ue%>&tipo_solicitud=libera_capacitacion">administrador</a>
                        <%}//else
                        } catch(SQLException e){out.print("exception"+e);}
