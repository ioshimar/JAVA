...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
  ...
  try{       
    PreparedStatement pst5=(PreparedStatement)conexion.conn.prepareStatement("SELECT nom_ent, \"INSNOMBRECIUDADMPIO\" as municipio, \"INSNOMBRE\" as nom_ue FROM \"seguim_CURT\".concertacion O, \"seguim_CURT\".ufg_directori_ufg C WHERE C.\"Id\" = O.id_ue AND C.\"AMBITO\" LIKE 'M%' AND O.fec_pri_env between '"+f_inicio_m+"' and '"+f_fin_m+"' ORDER BY nom_ent ASC;"  ,ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE); 
    ...
    XSSFCell celdaSubtitulo = FilaSubtitulo.createCell(1);
        celdaSubtitulo.setCellValue("Informe de oficios enviados a las Unidad del Estado del "+f_inicio_m+" al "+f_fin_m+"");
  ...
  try{       
    PreparedStatement pst6=(PreparedStatement)conexion.conn.prepareStatement("SELECT nom_ent, count(id_ue) as total FROM \"seguim_CURT\".concertacion O, \"seguim_CURT\".ufg_directori_ufg C WHERE C.\"Id\" = O.id_ue AND C.\"AMBITO\" LIKE 'M%' AND O.fec_pri_env between '"+f_inicio_m+"' and '"+f_fin_m+"' GROUP BY nom_ent ORDER BY nom_ent ASC;"  ,ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE); 
  ...
  XSSFRow filaX3 = hoja1.createRow((short)fv2);
   ...
   //Creo la celda A0
    XSSFCell CeldaA0 = fila.createCell(0);
        ...
        String nombre="reporte oficios recibidos municipal.xlsx";
