...
ResultSet rs1 = Estamento.executeQuery("SELECT count (id_ue) as tot_env FROM \"seguim_CURT\".registro_ue a, \"seguim_CURT\".ufg_directori_ufg b, \"seguim_CURT\".cat_edo c WHERE b.\"Id\" = a.id_ue AND b.\"CVE_ENTIDAD\" = c.\"CVE_EDO\" AND a.fec_sol_reg between '"+f_inicio_reg+"' and '"+f_fin_reg+"';");
...
tot_env = rs1.getString("tot_env");
        ...
        <!--<div id="cont_consulta">-->
            <span class="CarroisGR16">Informe de la instrumentación  de la CURT por Unidad del Estado del <%=f_inicio_reg%> al <%=f_fin_reg%></span><br><br>
            <div class="r_tit"> UNIDADES DEL ESTADO REGISTRADAS EN EL APLICATIVO: <%=tot_env%></div> 
            
         <%             


            ResultSet rs = Estamento.executeQuery("SELECT c.\"NOM_EDO\" as nom_ent,\"INSNOMBRECIUDADMPIO\" as municipio, \"INSNOMBRE\" as nom_ue FROM \"seguim_CURT\".registro_ue a, \"seguim_CURT\".ufg_directori_ufg b, \"seguim_CURT\".cat_edo c WHERE b.\"Id\" = a.id_ue AND b.\"CVE_ENTIDAD\" = c.\"CVE_EDO\" AND a.fec_sol_reg between '"+f_inicio_reg+"' and '"+f_fin_reg+"' ORDER BY c.\"NOM_EDO\" ASC;");
         ...
         %>
           ...
           %>            
            ...
            ResultSet rs2 = Estamento.executeQuery("SELECT c.\"NOM_EDO\" as nom_ent,count(id_ue) as total FROM \"seguim_CURT\".registro_ue a, \"seguim_CURT\".ufg_directori_ufg b, \"seguim_CURT\".cat_edo c WHERE b.\"Id\" = a.id_ue AND b.\"CVE_ENTIDAD\" = c.\"CVE_EDO\" AND a.fec_sol_reg between '"+f_inicio_reg+"' and '"+f_fin_reg+"' GROUP BY nom_ent ORDER BY nom_ent ASC;");
         ...
         %>
           ...
           <div id="cur_notas"><a href="exportar_reporte_reg_app.jsp?f_inicio_reg=<%=f_inicio_reg%>&f_fin_reg=<%=f_fin_reg%>" target="_new">Descargar archivo</a></div>
    </body>
</html>
