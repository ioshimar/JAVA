...
String f_inicio_ue2 = request.getParameter("f_inicio_ue2");
String f_fin_ue2 = request.getParameter("f_fin_ue2");
//String f_inicio = "2014-06-01";
//String f_fin = "2018-12-31"; 



ResultSet rs1 = Estamento.executeQuery("SELECT (a.cont+b.cont+c.cont) as tot_env FROM (SELECT count(id_ue) as cont FROM \"seguim_CURT\".concertacion WHERE fec_pri_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"') AS a, (SELECT count(id_ue) as cont FROM \"seguim_CURT\".concertacion WHERE fec_seg_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"') AS b, (SELECT count(id_ue) as cont FROM \"seguim_CURT\".concertacion WHERE fec_ter_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"') AS c;");
...
tot_env = rs1.getString("tot_env");
        ...
        <!--<div id="cont_consulta">-->
            <span class="CarroisGR16">Informe de la instrumentación  de la CURT por Unidad del Estado del <%=f_inicio_ue2%> al <%=f_fin_ue2%></span><br><br>
            <div class="r_tit"> RESPUESTAS RECIBIDAS TOTAL: <%=tot_env%></div> 
            
         <%             


            ResultSet rs = Estamento.executeQuery("SELECT nom_ent, \"INSNOMBRECIUDADMPIO\" as municipio, nom_ue, acepto FROM \"seguim_CURT\".concertacion O, \"seguim_CURT\".ufg_directori_ufg C WHERE C.\"Id\" = O.id_ue AND ((O.fec_pri_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"') OR (O.fec_seg_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"') OR (O.fec_ter_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"')) ORDER BY O.nom_ent ASC;");
         ...
         %>
           ...
           <%
               ...
               out.println("<table>");
				out.println("<thead>");
					out.println("<tr class='backrep'><th><span class='Gotham-Book'>ENTIDAD</span></th><th><span class='Gotham-Book'>MUNICIPIO</span></th><th><span class='Gotham-Book'>UNIDAD DEL ESTADO</span></th><th><span class='Gotham-Book'>ACEPTÓ</span></th></tr>");
                                      
                                out.println("</thead>");
				out.println("<tbody>");
                                while(rs.next()) {
					out.println("<tr><td><span class='CarroisGR'>"+rs.getString("nom_ent")+"</span></td><td><span class='CarroisGR'>"+rs.getString("municipio")+"</span></td><td><span class='CarroisGR'>"+rs.getString("nom_ue")+"</span></td><td><span class='CarroisGR'>"+rs.getString("acepto")+"</span></td></tr>");
                                        };
           ...
           %>            
            ...
            ResultSet rs2 = Estamento.executeQuery("SELECT nom_ent, count(nom_ue) as total FROM \"seguim_CURT\".concertacion O, \"seguim_CURT\".ufg_directori_ufg C WHERE C.\"Id\" = O.id_ue AND ((O.fec_pri_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"') OR (O.fec_seg_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"') OR (O.fec_ter_res between '"+f_inicio_ue2+"' and '"+f_fin_ue2+"')) GROUP BY nom_ent ORDER BY O.nom_ent ASC;");
         ...
         %>
           ...
           <div id="cur_notas"><a href="exportar_reporte_respuestas.jsp?f_inicio_ue2=<%=f_inicio_ue2%>&f_fin_ue2=<%=f_fin_ue2%>" target="_new">Descargar archivo</a></div>
    </body>
</html>
