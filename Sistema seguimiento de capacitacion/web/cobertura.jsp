 ...
 String id_ue = request.getParameter("ue");
    ...
    <head>
        ...
        <script src="header_menu/jquery-latest.min.js" type="text/javascript"></script>
          ...
          <script src="js/ajax_cobertura.js" type="text/javascript"></script>
          
          
          <script>
                $(document).ready(function(){  
                    var i=1;  
                    $('#add_cobertura').click(function(){  
                         i++;  
                         $('#dynamic_field_cob').append('<tr id="row'+i+'">\n\
                                                      <td><input type="text" name="municipio" class="w30" placeholder="Municipio"></td>\n\
                                                      <td><input type="text" name="nom_shape" id="tamano" class="w30" placeholder="Nombre del shape"></td>\n\
...
<link href="header_menu/menu_responsive.css" rel="stylesheet" type="text/css"/>
  ...
  String Nombre_carpeta="";
                      try{
       PreparedStatement pst1 =(PreparedStatement) conexion.conn.prepareStatement( "SELECT nom_archivo FROM \"seguim_CURT\".datos_entrega_fis WHERE id_entrega= '"+id_entrega+"' AND id_ue = '"+id_ue+"' ");
             ...
             <div id="cont_center2">
              ...
              <table class="table table-bordered" id="dynamic_field_cob"> 
                   <tr> 
                      ...
                      <!--  <th width="0px"></th>-->
                      <!--  <th width="0px"></th>-->
                       <th width="35%">Municipio</th>
                       <th width="35%">Nombre shape</th>
                       <th width="20%">Número de registros</th>
                   </tr>
                   <tr>
                     
                      <!-- <td width="0px"><input type="hidden" name="id_entrega" id="id_entrega" value="< %=id_entrega%>" ></td>-->
                      <!-- <td width="0px"><input type="hidden" name="id_ue" id="id_ue"  value="< %=id_ue%>"></td> -->
                       <td><input type="text" name="municipio" class="w30" placeholder="Municipio"></td> 
                       <td><input type="text" name="nom_shape" id="tamano" class="w30" placeholder="Nombre del shape"></td> 
                       <td><input type="text" name="no_reg" id="no_reg" class="w10" placeholder="No. de registros"></td> 
                        
                        <td><button type="button" name="add_cobertura" id="add_cobertura" class="btn btn-success">Agregar <br> renglones</button></td>
                   </tr>
               </table>
                  ...
                  <div id="btn_enviar">
                    ...
                    <input type="button" value="Guardar" onclick="javascript:proceso_cobertura();">
                 </div>
            </form>
        </div>
                       ...
                       <%
       try{
       PreparedStatement pst2 =(PreparedStatement) conexion.conn.prepareStatement( "SELECT id_cob, id_entrega, id_ue, municipio, nom_shape, num_reg FROM \"seguim_CURT\".cobertura"
               + " WHERE id_entrega= '"+id_entrega+"' AND id_ue = '"+id_ue+"'" ,ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
       conexion.rs =pst2.executeQuery(); 
      
                       %>
                       
      <div id="cont_tablas" >
          <span class="text-eliminar">Documentos almacenados en</span> <span class="capa-eliminar"> <%=Nombre_carpeta%> </span><br><br>
        <table id="lis_filas" style="max-width:600px; margin: 0 auto;" > 
            
           <%if(conexion.rs.next()){ //SI ENCUENTRA REGISTROS...%> 
             <tr>
               <td class="cabeReg" style="font-weight: bold;">Municipio </td>
               <td class="cabeReg" style="font-weight: bold;"> Nombre del Shape </td>
            ...
            <%
                              ...
                              conexion.rs.beforeFirst(); //ESTE REGRESA el cursor a la primera fila que devuelve el ResulsSet (lo resetea)..., pero
                                ...
                                while(conexion.rs.next()){
                                        int id_cob = conexion.rs.getInt("id_cob");
                                        String municipio =conexion.rs.getString("municipio");
                                        if(municipio==null)
                                            municipio="";
                                        String nom_shape=conexion.rs.getString("nom_shape");
                                        if(nom_shape==null)
                                            nom_shape="";
                                        String num_reg=conexion.rs.getString("num_reg");
                                        if(num_reg==null)
                                            num_reg="";
                            
                            %>
                            <tr class="hov">
                                <td class="ambito" align="center"><%out.print(municipio); %></td>
                                <td class="ambito" align="center"><%out.print(nom_shape); %></td>
                                <td class="ambito" align="center"><%out.print(num_reg); %></td>
                                <td class="center"> 
                                    <a href="actualiza_reg_cobertura.jsp?cob=<%=id_cob%>&ue=<%=id_ue%>&entrega=<%=id_entrega%>"> <span class="icon-edit"></span><span class="text-e">Editar </span></a>
                                </td>
                               
                                <td class="center"> 
                                    <a href="elimina_reg_cobertura.jsp?cob=<%=id_cob%>&ue=<%=id_ue%>&entrega=<%=id_entrega%>"><span class="icon-trashcan"></span><span class="text-e">  Eliminar</span></a>
