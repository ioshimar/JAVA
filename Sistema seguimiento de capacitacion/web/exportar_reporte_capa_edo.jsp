...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
    ...
    %>
        ...
        XSSFSheet hoja1 = objLibro.createSheet("Informe de capacitaciones "+cve_ent+"");// Creo la hoja
     ...
     static String id_concer, id_ue, nom_ent, cve_ent, nom_ue, fec_pri_env, fec_pri_res, concurt, sincurt,instituciones,num_pers,doc,  
  ...
  try{       
    PreparedStatement pst5=(PreparedStatement)conexion.conn.prepareStatement("SELECT T1.fecha_cap,count(*) as num_institu, sum(T1.cant_pers) as personas, string_agg(T2.\"INSNOMBRE\"::text, ', ' order by T2.\"INSNOMBRE\") as instituciones, ((case when T1.norma_cat = '' then '' else 'Norma de Catastro' end ) || ' - ' || (case when T1.norma_curt = '' then '' else 'Norma de la CURT' end )|| ' - ' || (case when T1.lineamiento = '' then '' else 'Lineamientos para el intercambio de información' end )|| ' - ' || (case when T1.diccionario = '' then '' else 'Diccionarios 1:1,000 y 1:10,000' end )) as doc "
                    + "FROM seguimiento_cap.capacitacion AS T1, seguimiento_cap.ufg_directori_ufg as T2 WHERE T2.\"Id\"  =  T1.id_ue AND T2.\"CVE_ENTIDAD\"='"+cve_ent+"' GROUP BY T1.fecha_cap,doc;"  ,ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE); 
    ...
    XSSFCell celdaSubtitulo = FilaSubtitulo.createCell(1);
        celdaSubtitulo.setCellValue("Informe de Unidades del Estado que han generado CURT");
    ...
    */    
     ...
     fila8.setHeight((short)0x190);
        celdaDatIE.setCellValue("CAPACITACIONES DE "+nom_ent.toUpperCase()+"");
   ...
   while(conexion.rs.next()){
       ...
       fecha_cap= conexion.rs.getString("fecha_cap");
        ...
        municipio= conexion.rs.getString("num_institu");
         if(municipio==null ){municipio="";}  
         
        instituciones= conexion.rs.getString("instituciones");
         if(instituciones==null ){instituciones="";}
         
        num_pers= conexion.rs.getString("personas");
         if(num_pers==null ){num_pers="";}
         
        doc= conexion.rs.getString("doc");
         if(doc==null ){doc="";}  
  
     String[] datosP5 = {fecha_cap,municipio,instituciones,num_pers,doc};
   ...
   //Creo la celda A0
    XSSFCell CeldaA0 = fila.createCell(0);
        ...
        String nombre="reporte de numero de capacitaciones "+nom_ent.toUpperCase()+".xlsx";
