...
<%@page contentType="text/html" pageEncoding="UTF-8"%>
    ...
    %>
     ...
     static String id_concer, id_ue, nom_ent, cve_ent, nom_ue, fec_pri_env, fec_pri_res, concurt, sincurt,  
  ...
  try{       
    PreparedStatement pst5=(PreparedStatement)conexion.conn.prepareStatement("SELECT D.\"NOM_EDO\" as nom_ent, \"INSNOMBRECIUDADMPIO\" as municipio, \"INSNOMBRE\" as nom_ue, sum(pred_concurt) as concurt, sum(pred_sincurt) as sincurt FROM \"seguim_CURT\".generacion_curt O, \"seguim_CURT\".ufg_directori_ufg C, \"seguim_CURT\".cat_edo D WHERE C.\"Id\" = O.id_ue AND C.\"CVE_ENTIDAD\" = D.\"CVE_EDO\" AND O.fech_fin between '"+f_inicio_gen+"' and '"+f_fin_gen+"' GROUP BY id_ue, D.\"NOM_EDO\", \"INSNOMBRECIUDADMPIO\", \"INSNOMBRE\" ORDER BY D.\"NOM_EDO\" ASC;"  ,ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE); 
    ...
    XSSFCell celdaSubtitulo = FilaSubtitulo.createCell(1);
        celdaSubtitulo.setCellValue("Informe de Unidades del Estado que han generado CURT "+f_inicio_gen+" al "+f_fin_gen+"");
    ...
    if(conexion.rs.next()){//SI encontró algo imprimimos lso encabezados...
     ...
     fila8.setHeight((short)0x190);
        celdaDatIE.setCellValue("GENERACIÓN DE CURT DE UNIDADES DEL ESTADO");
        ...
        XSSFCell celdaE10= fila10.createCell(4);
        celdaE10.setCellValue("CON CURT");
        celdaE10.setCellStyle(estiloCeldasBold);
        
        XSSFCell celdaF10= fila10.createCell(4);
        celdaF10.setCellValue("SIN CURT");
        celdaF10.setCellStyle(estiloCeldasBold);
   ...
   while(conexion.rs.next()){
       ...
       nom_ent= conexion.rs.getString("nom_ent");
        ...
        concurt= conexion.rs.getString("concurt");
         if(concurt==null ){concurt="";}
         
        sincurt= conexion.rs.getString("sincurt");
         if(sincurt==null ){sincurt="";}  
  
     String[] datosP5 = {nom_ent,municipio,nom_ue,concurt,sincurt};
  ...
  try{       
    PreparedStatement pst6=(PreparedStatement)conexion.conn.prepareStatement("SELECT D.\"NOM_EDO\" as nom_ent, count(DISTINCT id_ue) as total FROM \"seguim_CURT\".generacion_curt O, \"seguim_CURT\".ufg_directori_ufg C, \"seguim_CURT\".cat_edo D WHERE C.\"Id\" = O.id_ue AND C.\"CVE_ENTIDAD\" = D.\"CVE_EDO\" AND O.fech_fin between '"+f_inicio_gen+"' and '"+f_fin_gen+"' GROUP BY D.\"NOM_EDO\" ORDER BY D.\"NOM_EDO\" ASC;"  ,ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE); 
  ...
  XSSFRow filaX3 = hoja1.createRow((short)fv2);
   ...
   //Creo la celda A0
    XSSFCell CeldaA0 = fila.createCell(0);
        ...
        String nombre="reporte de Unidades del Estado que han generado CURT.xlsx";
