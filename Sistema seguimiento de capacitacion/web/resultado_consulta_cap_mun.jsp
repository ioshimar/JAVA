...
String f_inicio_cap_mun = request.getParameter("f_inicio_cap_mun");
String f_fin_cap_mun = request.getParameter("f_fin_cap_mun");
//String f_inicio = "2014-06-01";
//String f_fin = "2018-12-31"; 



ResultSet rs1 = Estamento.executeQuery("SELECT count(id_ue) as tot_env FROM \"seguim_CURT\".capacitacion O, \"seguim_CURT\".ufg_directori_ufg C, \"seguim_CURT\".cat_edo D WHERE C.\"Id\" = O.id_ue AND C.\"CVE_ENTIDAD\" = D.\"CVE_EDO\" AND C.\"AMBITO\" LIKE 'M%' AND O.fecha_cap between '"+f_inicio_cap_mun+"' and '"+f_fin_cap_mun+"';");
...
tot_env = rs1.getString("tot_env");
        ...
        <!--<div id="cont_consulta">-->
            <span class="CarroisGR16">Informe de la instrumentación  de la CURT por Unidad del Estado del <%=f_inicio_cap_mun%> al <%=f_fin_cap_mun%></span><br><br>
            <div class="r_tit"> CAPACITACIONES (MUNICIPAL) TOTAL: <%=tot_env%></div> 
            
         <%             


            ResultSet rs = Estamento.executeQuery("SELECT D.\"NOM_EDO\" as nom_ent, \"INSNOMBRECIUDADMPIO\" as municipio, \"INSNOMBRE\" as nom_ue, fecha_cap FROM \"seguim_CURT\".capacitacion O, \"seguim_CURT\".ufg_directori_ufg C, \"seguim_CURT\".cat_edo D WHERE C.\"Id\" = O.id_ue AND C.\"CVE_ENTIDAD\" = D.\"CVE_EDO\" AND C.\"AMBITO\" LIKE 'M%' AND O.fecha_cap between '"+f_inicio_cap_mun+"' and '"+f_fin_cap_mun+"' ORDER BY D.\"NOM_EDO\" ASC;");
         ...
         %>
           ...
           <%
               ...
               out.println("<table>");
				out.println("<thead>");
					out.println("<tr class='backrep'><th><span class='Gotham-Book'>ENTIDAD</span></th><th><span class='Gotham-Book'>MUNICIPIO</span></th><th><span class='Gotham-Book'>UNIDAD DEL ESTADO</span></th><th><span class='Gotham-Book'>FECHA DE CAPACITACIÓN</span></th></tr>");
                                      
                                out.println("</thead>");
				out.println("<tbody>");
                                while(rs.next()) {
					out.println("<tr><td><span class='CarroisGR'>"+rs.getString("nom_ent")+"</span></td><td><span class='CarroisGR'>"+rs.getString("municipio")+"</span></td><td><span class='CarroisGR'>"+rs.getString("nom_ue")+"</span></td><td><span class='CarroisGR'>"+rs.getString("fecha_cap")+"</span></td></tr>");
                                        };
         ...
         <div class="r_tit"> TOTALES </div>            
            ...
            ResultSet rs2 = Estamento.executeQuery("SELECT D.\"NOM_EDO\" as nom_ent, count(O.id_ue) as total FROM \"seguim_CURT\".capacitacion O, \"seguim_CURT\".ufg_directori_ufg C, \"seguim_CURT\".cat_edo D WHERE C.\"Id\" = O.id_ue AND C.\"CVE_ENTIDAD\" = D.\"CVE_EDO\" AND C.\"AMBITO\" LIKE 'M%' AND O.fecha_cap between '"+f_inicio_cap_mun+"' and '"+f_fin_cap_mun+"' GROUP BY D.\"NOM_EDO\" ORDER BY D.\"NOM_EDO\" ASC;");
         ...
         %>
           ...
           <div id="cur_notas"><a href="exportar_reporte_cap_mun.jsp?f_inicio_cap_mun=<%=f_inicio_cap_mun%>&f_fin_cap_mun=<%=f_fin_cap_mun%>" target="_new">Descargar archivo</a></div> 
