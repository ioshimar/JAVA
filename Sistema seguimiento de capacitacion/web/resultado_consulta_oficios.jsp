...
ResultSet rs1 = Estamento.executeQuery("SELECT count(id_ue) as tot_env FROM \"seguim_CURT\".concertacion O, \"seguim_CURT\".ufg_directori_ufg C WHERE C.\"Id\" = O.id_ue AND C.\"AMBITO\" LIKE 'F%' AND O.fec_pri_env between '"+f_inicio+"' and '"+f_fin+"';");
...
tot_env = rs1.getString("tot_env");
        ...
        <!--<div id="cont_consulta">-->
            <span class="CarroisGR16">Informe de la instrumentación  de la CURT por Unidad del Estado del <%=f_inicio%> al <%=f_fin%></span><br><br>
            <div class="r_tit"> OFICIOS ENVIADOS (FEDERAL) TOTAL: <%=tot_env%></div> 
            
         <%             


            ResultSet rs = Estamento.executeQuery("SELECT nom_ent, \"INSNOMBRECIUDADMPIO\" as municipio, \"INSNOMBRE\" as nom_ue FROM \"seguim_CURT\".concertacion O, \"seguim_CURT\".ufg_directori_ufg C WHERE C.\"Id\" = O.id_ue AND C.\"AMBITO\" LIKE 'F%' AND O.fec_pri_env between '"+f_inicio+"' and '"+f_fin+"' ORDER BY nom_ent ASC;");
 ...
 <%             
            ...
            ResultSet rs2 = Estamento.executeQuery("SELECT nom_ent, count(id_ue) as total FROM \"seguim_CURT\".concertacion O, \"seguim_CURT\".ufg_directori_ufg C WHERE C.\"Id\" = O.id_ue AND C.\"AMBITO\" LIKE 'F%' AND O.fec_pri_env between '"+f_inicio+"' and '"+f_fin+"' GROUP BY nom_ent ORDER BY nom_ent ASC;");
         ...
         %>
           ...
           %>            
           <div id="cur_notas"><a href="exportar_reporte_oficios_federal.jsp?f_inicio=<%=f_inicio%>&f_fin=<%=f_fin%>" target="_new">Descargar archivo</a></div>   
