...
public class cambia_pass extends HttpServlet {
    ...
    String usuario = "";
    String psw = "";
    PreparedStatement pst11=(PreparedStatement) conexion.conn.prepareStatement( "SELECT \"CVE_ENTIDAD\", \"ENCNOMBRE\", \"ENCPWD\", \"ENCESTATALES\", \"ENTIDAD_CHAR\" \n" +
"  FROM \"seguim_CURT\".\"Encargados_CURT\" WHERE \"CVE_ENTIDAD\" = '"+user+"' AND \"ENCPWD\" = '"+old_pass+"' ");
    conexion.rs =pst11.executeQuery(); 
       while(conexion.rs.next()) {
            usuario= conexion.rs.getString("CVE_ENTIDAD");
            psw = conexion.rs.getString("ENCPWD");
  // out.print("ambito "+selambito_ins);
    ...
    if(user.equals(usuario) && old_pass.equals(psw) && new_pass.equals(repeat_pass)){  //Si el usuario y la contraseña anterior que pone el usuario son las mismas que estan en la BD y si las contraseñas nueva y repetida coinciden(esto ultimo se puede omitir ya que esta validado en ajax_psw.js)
       conexion.stmt=conexion.conn.createStatement();      

       conexion.stmt.executeUpdate("UPDATE \"seguim_CURT\".\"Encargados_CURT\" SET  \"ENCPWD\"='"+new_pass+"', cambio_psw=CURRENT_DATE, hora_cambio=now() WHERE \"CVE_ENTIDAD\" = '"+user+"' AND \"ENCPWD\" = '"+old_pass+"'");
