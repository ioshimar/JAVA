...
"  FROM \"seguim_CURT\".concertacion  WHERE id_ue = ? ORDER BY id_concer";
    ...
    public List<FilasDTOp2> cargaCap(int id_cap) throws SQLException {
      //  DataSource ds = AdministradorDataSource.getDataSource();
        ...
        System.out.println("Aqui otr vez ----------------------------------"+id_cap);
      
      try{
        QueryRunner qr = new QueryRunner(ds);
        String sql = "SELECT id_cap, id_ue, TO_CHAR(fecha_cap, 'dd/MM/yyyy') as fecha_cap, lugar, resp_inegi, cant_pers, TO_CHAR(fecha_llenado, 'dd/MM/yyyy') as fecha_llenado, TO_CHAR(fecha_actualizacion, 'dd/MM/yyyy') as  fecha_actualizacion\n" +
"  FROM \"seguim_CURT\".capacitacion WHERE id_cap = ?";
    ...
    public List<FilasDTOp2> cargaCapacitacion(int id_ue) throws SQLException {
      ...
      try{
        QueryRunner qr = new QueryRunner(ds);
        String sql = "SELECT id_cap, id_ue, TO_CHAR(fecha_cap, 'dd/MM/yyyy') as fecha_cap, lugar, resp_inegi, cant_pers, TO_CHAR(fecha_llenado, 'dd/MM/yyyy') as fecha_llenado, TO_CHAR(fecha_actualizacion, 'dd/MM/yyyy') as  fecha_actualizacion\n" +
"  FROM \"seguim_CURT\".capacitacion WHERE id_ue = ? ORDER BY id_cap";
        Object[] params = {id_ue};        
        ResultSetHandler rsh = new BeanListHandler(FilasDTOp2.class);
